...
async def send_spinner_message(context: ContextTypes.DEFAULT_TYPE, chat_id: int, text_template: str = "Processing{}"):
    ...
    return await context.bot.send_message(chat_id, initial_text, parse_mode=ParseMode.HTML)
...
<pre>───────────────────────────────────</pre>
    {random.choice([f"Shopify Checker initialized, {user_name}.", f"Ready for operations, {user_name}."])}
...
<pre>───────────────────────────────────</pre>
    ...
    reply_markup = InlineKeyboardMarkup(keyboard_cmds)
...
<pre>───────────────────────────────────</pre>"""
        ...
        message_text = "⚠️ No Shopify site is currently set. Use /add <code>&lt;url&gt;</code>."
    
    if from_button and update.callback_query:
        await update.callback_query.message.edit_text(message_text, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Main Menu", callback_data="nav:show_start")]]))
    else:
...
async def chk_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or context.args[0].count('|') != 3:
        await update.message.reply_text("⚠️ Invalid format. Use: /chk <code>N|M|Y|C</code>", parse_mode=ParseMode.HTML)
        return
    ...
    b = bin_data
    bin_info_str = " - ".join(filter(lambda x: x != 'N/A' and x, [b.get('scheme'), b.get('type'), b.get('brand')])) or "N/A"
    ...
    result_message = (
        ...
        f"<pre>─ ─ ─ Meta ─ ─ ─</pre>\n"
        f"👤 <b>Checked By:</b> {html.escape(update.effective_user.username or update.effective_user.first_name)} ⚜️ [Elite]\n"
...
async def mchk_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result_file_content = "\n".join(results_log)
    result_filename = f"Results_{user_id}_{int(time.time())}.txt"
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data in ("chk:prompt_now", "chk:prompt_another"):
...
def main():
    ...
    # Combined handler for both reply and caption
